...
def _write_marker_helper(path: Path, environment_name: str, value: str) -> None:
    if os.name == "nt":
        ...
        path.write_text(
            f'#!/bin/sh\nprintf %s {value} > "${environment_name}"\n',
            encoding="utf-8",
        )
        path.chmod(0o755)


@pytest.mark.skipif(os.name == "nt", reason="POSIX child PATH lookup behavior")
def test_client_process_drops_relative_path_entries_before_helper_lookup(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    launcher = trusted_bin / "codex"
    launcher.write_text("#!/bin/sh\nhelper\n", encoding="utf-8")
    launcher.chmod(0o755)
    _write_marker_helper(trusted_bin / "helper", "TRUSTED_MARKER", "trusted")
    _write_marker_helper(relative_bin / "helper", "MALICIOUS_MARKER", "malicious")
...
@pytest.mark.skipif(os.name != "nt", reason="Windows command processor behavior")
def test_windows_batch_command_rejects_a_relative_command_processor(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    with pytest.raises(ValueError, match="absolute"):
        windows_batch_command(Path("C:/tools/codex.cmd"), ())
