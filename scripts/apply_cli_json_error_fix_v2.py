...
def apply() -> None:
    ...
    class_marker = "class JapaneseArgumentParser(argparse.ArgumentParser):\n"
    build_marker = "def _build_parser() -> argparse.ArgumentParser:\n"
    if text.count(class_marker) != 1 or text.count(build_marker) != 1:
        raise SystemExit("parser region anchor mismatch")

    if "def _failure_payload(" not in text:
        class_start = text.index(class_marker)
        text = text[:class_start] + HELPERS + text[class_start:]

    text = _replace_region(text, class_marker, build_marker, PARSER_CLASS)
    ...
    validate_marker = "def _run_validate(args: argparse.Namespace) -> int:\n"
    json_output_marker = "def _json_output(payload: JSONDict) -> None:\n"
    if text.count(validate_marker) != 1 or text.count(json_output_marker) != 1:
        raise SystemExit("validate region anchor mismatch")
    text = _replace_region(text, validate_marker, json_output_marker, NEW_VALIDATE + "\n\n")

    main_marker = "def main(argv: Sequence[str] | None = None) -> int:\n"
    module_marker = '\n\nif __name__ == "__main__":'
    if text.count(main_marker) != 1 or text.count(module_marker) != 1:
        raise SystemExit("main region anchor mismatch")
    text = _replace_region(text, main_marker, module_marker, NEW_MAIN)
