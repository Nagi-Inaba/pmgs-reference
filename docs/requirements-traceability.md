...
| AGENT-02 | 同じ読み取り専用skillを両clientへ非破壊で導入する | verified | hash一致、冪等、上書き拒否、途中失敗回収test |
...
| RELEASE-01 | 合成fixtureでrepository全検査を通す | verified | pytest 56件、Ruff、mypy、boundary、sdist、wheelに合格 |
...
GitHub source repositoryを既定の配布面とする。R2 upload、Worker deploy、domain設定、PyPI公開、外部indexは別の外部releaseとして記録する。
