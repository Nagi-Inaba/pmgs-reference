...
| STORE-02 | 内容アドレス付きSQLiteを保持し、検証済み現行版を原子的に切り替える | implemented | `data_paths.py`、`setup.py`、setupの再利用・source変更・lock・legacy・pointer test。全OS wheel CIは次回hosted runで確認する |
...
| CLI-03 | `pmgs setup`が全OSで構築、検証、切替、client接続を一つの入口から実行する | implemented | setup CLI、JSON・対話契約、dry-run、wheel E2E test。hosted macOSを含むCIは外部確認待ち |
...
| AGENT-05 | client登録を管理ディレクトリ参照にし、同一設定を再利用して競合を上書きしない | implemented | fake Codex・Claude Codeのargv、再利用、競合、部分失敗、`CLAUDE_CONFIG_DIR`、Windows batch安全性test |
...
| RELEASE-04 | wheelを3 OSで隔離導入し、setup、再実行、doctorを検証する | implemented | `scripts/verify_wheel_install.py`と3 OSの`wheel-e2e` job。hosted結果は外部確認待ち |
