...
| RELEASE-04 | wheelとsdistを3 OSで隔離導入し、setup、再実行、doctorを検証する | implemented | 通常CIとtag releaseの双方へ、同一artifactのwheelとsdist由来wheelをLinux、Windows、macOSで検証するgateを追加。hosted PR CIの成功後にverifiedへ更新する |
...
GitHub source repository、[PyPIのv0.4.0](https://pypi.org/project/pmgs-reference/0.4.0/)、[GitHub Release v0.4.0](https://github.com/Nagi-Inaba/pmgs-reference/releases/tag/v0.4.0)を現在の配布面とし、v0.5.0はrelease candidateとしてtag公開を待つ。R2 upload、Worker deploy、domain設定、外部indexは別の外部releaseとして記録する。
